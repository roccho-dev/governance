...
bash "$TOOL" build --input "$FIX/accepted.jsonl" --repo roccho-dev/governance --required-root purpose:company:high-value-sale --audience public --out "$TMP/two"
...
if bash "$TOOL" build --input "$FIX/no-accepted-decision.jsonl" --repo roccho-dev/governance --required-root purpose:company:high-value-sale --audience public --out "$TMP/case1" 2>/dev/null; then
  echo "negative fixture unexpectedly passed: no-accepted-decision" >&2
  exit 1
fi
if bash "$TOOL" build --input "$FIX/wrong-root.jsonl" --repo roccho-dev/governance --required-root purpose:company:high-value-sale --audience public --out "$TMP/case2" 2>/dev/null; then
  echo "negative fixture unexpectedly passed: wrong-root" >&2
  exit 1
fi
if bash "$TOOL" build --input "$FIX/purpose-cycle.jsonl" --repo roccho-dev/governance --required-root purpose:company:high-value-sale --audience public --out "$TMP/case3" 2>/dev/null; then
