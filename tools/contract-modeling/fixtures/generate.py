...
LEGACY_DIGEST = hashlib.sha256(
    json.dumps(LEGACY_IDS, sort_keys=True, separators=(",", ":")).encode()
).hexdigest()
assert LEGACY_DIGEST == "be736567e5fb4751dfd53a272aeede308c38f9da9d51d18a60bcc2d74f946b07"
...
def receipt(receipt_id, effectful):
    return {
        "receipt_id": receipt_id,
...
def build():
    rows = []
    purpose_nodes = [("M0", True), ("P4", False), ("P3", False), ("P2", False), ("P1", False), ("P0", False)]
    for purpose_id, is_root in purpose_nodes:
    ...
    for child, parent in [("P4", "M0"), ("P3", "P4"), ("P2", "P3"), ("P1", "P2"), ("P0", "P1")]:
    ...
    for claim_id, subject, payload in [
        ("package-contract:code-governance", "contract:pkg:governance:code-governance",
         package("pkg:governance:code-governance", "receipt:code-governance", "canonical-ledger.v1", "semantic-packet.v1", ["filesystem-read"])),
        ("package-contract:claim-admission", "contract:pkg:governance:claim-admission",
         package("pkg:governance:claim-admission", "receipt:claim-admission", "claim-port-input.v1", "claim-port-admission.v1", ["filesystem-read"])),
        ("package-contract:model-only", "contract:pkg:governance:model-only-transform",
         package("pkg:governance:model-only-transform", "receipt:model-only", "model-only.input.v1", "model-only.output.v1", ["none"])),
    ]:
        rows.append(row(claim_id, "claim", "package-contract-v1", "package-contract", subject, payload))

    requests = [
        model("01-extend", "model:customer", ["customer_id"], ["customer_id", "name"], ["customer"], witnesses=["existing-use"]),
        model("02-semantic", "model:customer", ["customer_id"], ["customer_id", "name", "alias"], [], witnesses=["vocabulary-use"]),
        model("03-projection", "model:customer", ["customer_id"], ["customer_id", "name", "search"], ["customer-search"], projection="customer-search-v1", witnesses=["search-use"]),
        model("04-query", "model:customer", ["customer_id"], ["customer_id", "email"], ["email-search"], queries=["customer-by-email"], witnesses=["query-use"]),
        model("05-destructive", "model:customer", ["customer_id"], ["customer_id"], ["customer"], coverage="total", round_trip=False, witnesses=["breaking-use"]),
        model("06-new", "new:risk-assessment", ["assessment_id"], ["assessment_id", "customer_id", "score"], ["risk-assessment"], witnesses=["multiple-assessments"], distinct=["independent-lifecycle"]),
        model("07-ambiguous", "model:customer-account-link", ["customer_id"], ["customer_id", "account_id"], ["customer-account-link"]),
        model("08-raw", "model:customer", ["customer_id"], ["customer_id", "name"], ["customer"], queries=["raw-scan"], raw=True, witnesses=["request"]),
    ]
    for payload in requests:
        request_id = payload["request_id"]
        rows.append(row(f"request:{request_id}", "claim", "data-model-v1", "model-request", f"request:{request_id}", payload))

    for receipt_id, effectful in [("receipt:code-governance", True), ("receipt:claim-admission", True), ("receipt:model-only", False)]:
        rows.append(row(f"effect-receipt:{receipt_id}", "receipt", "effect", "effect-receipt", receipt_id, receipt(receipt_id, effectful)))
    ...
    for legacy_id in LEGACY_IDS:
        ...
        legacy_row = row(f"legacy:{legacy_id}", "legacy-mapping", "migration", "legacy-responsibility", f"legacy:{legacy_id}", payload)
...
def main():
    ...
    print(json.dumps({"claims": len(rows), "legacy": len(legacy_rows), "legacyDigest": LEGACY_DIGEST}, sort_keys=True))
