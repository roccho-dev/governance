...
def main():
    ...
    feat={'kind':'feat.input.v1','packageId':args.package,'specId':r['specId'],'status':'ready' if r['status']=='accepted' else 'planned-blocked','sourceAuthority':'governance-records-main/records/specs/package-contract.v1.jsonl','rawAdrDirectAuthority':False,'projectionDigest':projection_digest,'repoOperation':{'targetRepoId':d.get('successorRepoId'),'repoSourceUri':d.get('repoSourceUri'),'allowedPaths':d.get('allowedPaths') or [],'forbiddenPaths':d.get('forbiddenPaths') or []},'environmentBuildDefinition':{'kind':'governance.environmentBuildDefinition.v1','sourceOfTruth':'package-contract.v1.jsonl','package':args.package,'targetPackage':args.package,'repoId':d.get('successorRepoId'),'officialOutput':d.get('officialOutput'),'requiredOutputs':d.get('requiredOutputs') or [],'requiredChecks':d.get('requiredChecks') or [],'requiredCommands':d.get('requiredCommands') or [],'runtimeRequirements':d.get('runtimeRequirements'),'preflightRequiredTools':d.get('preflightRequiredTools') or []},'dependencyLock':edges}
