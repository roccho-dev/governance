...
POLICY = ROOT / "tools/promotion/policy-shadow.v1.json"
BASELINE = ROOT / "governance/signed-promotion-baseline.v1.json"
ENGINE_FILES = ["tools/promotion/core.py","tools/promotion/ports.py","tools/promotion/adapters.py","tools/promotion/cli.py"]
...
def engine_digest() -> str:
    ...
    for relative in ENGINE_FILES:
        data=(ROOT/relative).read_bytes(); manifest.append({"path":relative,"sha256":hashlib.sha256(data).hexdigest()})
    return digest(manifest)


def validate_baseline(value: dict[str, Any]) -> None:
    need(value.get("kind")=="governance.signedPromotionBaseline.v1","baseline-kind"); need(value.get("status")=="current","baseline-status"); need(value.get("closureModel")=="signed-promotion","baseline-closure-model"); need(value.get("supersedesClosureModel")=="github-merge-protection","baseline-supersedes")
    need(value.get("historicalClosureReceipts")=={"adrs223":"superseded","adrs233":"superseded","governance150":"superseded"},"baseline-old-closure"); need(value["governance"]["workflowCount"]==2,"baseline-workflow-count")
    need(value["invariants"]["githubMergeHasAuthority"] is False,"baseline-github-merge"); need(value["invariants"]["githubRulesetRequired"] is False,"baseline-ruleset"); need(value["invariants"]["allRepositoriesEnforced"] is False,"baseline-all-repositories"); need(value["invariants"]["businessOutcomeAchieved"] is False,"baseline-business-outcome")


def validate_live(value: dict[str, Any]) -> tuple[str, str]:
    need(value.get("kind")=="governance.liveSelectedConsumerPacket.v1","live-kind"); need(value.get("status")=="pass","live-status"); need(value.get("artifactBodiesVerified") is True,"live-artifact-bodies"); need(value.get("receiptCandidateShaBound") is True,"live-receipt-sha")
    rows=value.get("repositories"); need(isinstance(rows,list) and len(rows)==2,"live-repository-count"); need(len({row.get("repository") for row in rows if isinstance(row,dict)})==2,"live-duplicate-repository")
    claims=[]; receipts=[]
...
def candidate(gate: dict[str, Any], live: dict[str, Any], candidate_sha: str) -> dict[str, Any]:
    baseline=read(BASELINE); policy_value=read(POLICY); validate_baseline(baseline); policy=Policy.from_dict(policy_value); need(policy.production_key_provisioned is False,"shadow-production-key"); need(policy.accepted_decision_status=="proposed","shadow-decision-status")
    claim_set,receipt_set=validate_live(live)
    packet=make_promotion_candidate(candidate_digest=candidate_sha,accepted_decision_digest=policy.contract_digest,gate_report=gate,claim_set_digest=claim_set,receipt_set_digest=receipt_set,engine_digest=engine_digest(),accepted_decision_status=policy.accepted_decision_status)
    need(packet["promotionAdmission"] is False and packet["decision"]=="shadow-allow","shadow-admission")
...
def selftest() -> dict[str, Any]:
    gate,live=fixture(); packet=candidate(gate,live,"a"*40); cases=[]
    bad=copy.deepcopy(gate); bad["candidateSha"]="b"*40; cases.append(reject("gate-other-candidate",lambda:candidate(bad,live,"a"*40)))
    bad=copy.deepcopy(live); bad["artifactBodiesVerified"]=False; cases.append(reject("artifact-body-unverified",lambda:candidate(gate,bad,"a"*40)))
    bad=copy.deepcopy(live); bad["repositories"][0]["receipt"]["candidateSha"]="9"*40; cases.append(reject("receipt-other-candidate",lambda:candidate(gate,bad,"a"*40)))
    bad=copy.deepcopy(live); bad["repositories"].append(copy.deepcopy(bad["repositories"][0])); cases.append(reject("duplicate-consumer",lambda:candidate(gate,bad,"a"*40)))
    bad=read(BASELINE); bad["historicalClosureReceipts"]["governance150"]="completed"; cases.append(reject("old-closure-active",lambda:validate_baseline(bad)))
    workflows="\n".join(path.read_text(encoding="utf-8") for path in (ROOT/".github/workflows").glob("*.yml")); need("tools.promotion.cli publish" not in workflows and "--private-key-file" not in workflows,"candidate-workflow-publisher"); need(not list(ROOT.glob("**/*.key")),"private-key-in-repository")
