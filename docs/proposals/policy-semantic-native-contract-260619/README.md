...
| projected policy entry | `policy-entry.accepted.env`, `policy.md`, and `rules/*.md` for bootstrap projected mode |
...
- Projected policy entry directories generated by the candidate compiler must
  set `POLICY_ENTRY_ACCEPTED=false` unless a future governance acceptance gate
  has approved and locked the artifact. Fixture accepted output is allowed only
  for bootstrap projected-mode tests and must be marked fixture-only.
...
| projected-policy-entry-accepted | remains blocked; no real accepted projected artifact exists |
